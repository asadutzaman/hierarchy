...
<div class="container-fluid">
    ...
    <div class="card mb-4">
        ...
        <table class="table table-bordered table-striped">
            ...
            @foreach($data as $row)
            <tr>
                <td><img src="{{ URL::to('/') }}/images/news_and_events/{{ $row->image }}" class="img-thumbnail" width="75" /></td>
                <td>{{ $row->headline }}</td>
                <td>{{ $row->created_at }}</td>
                <td>
                    
                    <form action="{{ route('admin_news_and_event.destroy', $row->id) }}" method="post">
                        <a href="{{ route('admin_news_and_event.show', $row->id) }}" class="btn btn-outline-success">
                            <i class="fa fa-info-circle" aria-hidden="true"></i>
                        </a>
                        <a href="{{ route('admin_news_and_event.edit', $row->id) }}" class="btn btn-outline-warning">
        ...
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Add New News And Event</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form method="post" action="{{ route('admin_news_and_event.store') }}" enctype="multipart/form-data">
                    @csrf
